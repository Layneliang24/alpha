...
class ArticleView(NavView, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user_file_count'] = models.File.objects.filter(author_id=self.request.user.id).count()
        context['user_link_count'] = models.Link.objects.filter(author_id=self.request.user.id).count()
        return context


@method_decorator(login_required(login_url="login:login"), name='dispatch')
class VisitView(DetailView):
    # 模板位置
    model = User
    ...
    def get_object(self, queryset=None):
        ...
        """
        # 首先调用父类的方法
        user = super(VisitView, self).get_object()
        return user
    ...
    def get_context_data(self, **kwargs):
        ...
        context['profile'] = login_models.Profile.objects.get(user_id=self.request.user.id)
